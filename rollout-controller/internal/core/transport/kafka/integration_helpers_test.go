...
func produceMessage(t *testing.T, topic string, key, value []byte) {
	...
	requireNoErr2(t, writer.WriteMessages(ctx, kafka.Message{Key: key, Value: value}))
}

func requireNoErr2(t *testing.T, err error) {
	t.Helper()
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func readMatchingFromTopic(t *testing.T, topic string, timeout time.Duration, match func(kafka.Message) bool) (kafka.Message, bool) {
	...
	reader := kafka.NewReader(kafka.ReaderConfig{
		Brokers:     []string{testBrokerAddr},
		Topic:       topic,
		GroupID:     "dlq-reader-" + topic + "-" + time.Now().Format("150405.000"),
