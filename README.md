...
- **Kafka** — асинхронный пайплайн событий: топик `device.checkins` с ключом `device_id` и топик `firmware.update-results` с ключом `campaign_id`. Все результаты одной кампании попадают в одну партицию, поэтому счётчики меняются в порядке публикации событий.
...
6. Rollout Controller асинхронно агрегирует результаты из Kafka в счётчики Redis и отдаёт их через `GET /campaigns/{id}` в поле `stats`.
