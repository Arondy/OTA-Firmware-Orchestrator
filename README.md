...
> [!NOTE]
> Повторный checkin устройства, уже сидящего на версии кампании, всегда даёт «нет обновления» — решение по версии, а не по факту участия в раскатке. `stage_id` из ответа checkin устройство возвращает в report без изменений, поэтому результат приписывается той стадии, на которой выдано обновление.
